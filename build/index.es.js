...
class OdataProvider {
  constructor(_options = {}) {
    _defineProperty(this, "callApi", null);

    _defineProperty(this, "groupCountFieldName", 'childCount');

    _defineProperty(this, "isCaseSensitiveStringFilter", false);

    _defineProperty(this, "beforeRequest", null);

    _defineProperty(this, "beforeSetSecondaryColumns", null);

    _defineProperty(this, "afterLoadData", null);

    _defineProperty(this, "odataOperator", {
      // Logical
      ...
      inStr: (col, values, isCaseSensitiveStringFilter) => `${this.ifTolowerCol(col, isCaseSensitiveStringFilter)} in (${values.map(x => `'${this.ifTolower(x, isCaseSensitiveStringFilter)}'`).join()})`,
    ...
    _defineProperty(this, "ifTolowerCol", (col, isCaseSensitiveStringFilter) => isCaseSensitiveStringFilter ? col : `tolower(${col})`);

    _defineProperty(this, "ifTolower", (value, isCaseSensitiveStringFilter) => isCaseSensitiveStringFilter ? value : value ? value.toLowerCase() : value);

    _defineProperty(this, "odataAggregation", {
      // Logical
    ...
    _defineProperty(this, "toQuery", options => {
      let path = [];
    ...
    _defineProperty(this, "encode", value => value ? value.replace("'", "''") : value);

    _defineProperty(this, "toDateTime", value => `${value}T00:00:00.000Z`);

    _defineProperty(this, "getFilterOdata", (colName, col) => {
      ...
      switch (col.filterType) {
        ...
        case 'text':
          {
            ...
            const filter = me.encode(col.filter); // let filterTo = me.encode(col.filterTo);

            if ((operatorName === 'equals' || operatorName === 'notEqual') && !me.isCaseSensitiveStringFilter) {
              operatorName += 'Str';
            }
        ...
        case 'set':
          return col.values.length > 0 ? me.odataOperator.inStr(colName, col.values) : null;
      }
    });

    _defineProperty(this, "getPivot", (pivotCols, rowGroupCols, valueCols, data, countField) => {
    ...
    _defineProperty(this, "buildGroupsFromData", (rowData, rowGroupCols, groupKeys, countField) => {
      const me = this;
    ...
    _defineProperty(this, "iterateObject", (object, callback) => {
      if (!object) {
        return;
      }
    ...
    _defineProperty(this, "groupBy", (data, field) => {
      var result = {};
      data.forEach(function (item) {
        var key = item[field];
    ...
    _defineProperty(this, "aggregateList", (rowData, countField) => {
      var result = {};
      rowData.forEach(row => {
    ...
    _defineProperty(this, "getFilterValuesParams", (field, callback) => {
      const me = this;
    ...
    _defineProperty(this, "getRows", params => {
      const me = this;
      ...
      const pivotActive = !isServerMode ? false : request.pivotMode && request.pivotCols.length > 0 && request.valueCols.length > 0;
      ...
      if (isServerMode) {
        if (request.rowGroupCols.length > 0) {
          ...
          if (request.groupKeys.length < request.rowGroupCols.length) {
            ...
            if (pivotActive) {
              groups = groups.concat(request.pivotCols.map(x => me.getWrapColumnName(x.field)));
              groups.forEach(x => {
            ...
            for (let idx = 0; idx < request.groupKeys.length; idx++) {
              const colValue = request.groupKeys[idx];
              const condition = `${me.getWrapColumnName(request.rowGroupCols[idx].field)} eq '${colValue}'`;
      ...
      me.callApi(query).then(async x => {
        if (!x) {
          ...
          if (!pivotActive) {
            ...
            let rowData = x; // Check count

            if (rowData.length === options.top && options.skip === 0 && request.groupKeys.length === 0) {
              let eof = false;
            ...
            const pivotResult = me.getPivot(request.pivotCols, request.rowGroupCols, request.valueCols, rowData, childCount);
            rowData = pivotResult.data;
            const secondaryColDefs = pivotResult.secondaryColDefs;
            rowData = me.buildGroupsFromData(rowData, request.rowGroupCols, request.groupKeys, childCount);
            const totalCount = request.groupKeys.length === 0 ? rowData.length : rowData.length === options.top ? null : rowData.length;

            if (totalCount > options.top) {
              const serverSideBlock = params.parentNode.rowModel.rowNodeBlockLoader.blocks[0];
    ...
    if (this.beforeRequest != null && typeof this.beforeRequest !== 'function') {
      throw new Error('beforeRequest must be a function');
    }

    if (this.afterLoadData != null && typeof this.afterLoadData !== 'function') {
